...
Supports resume from previous run via downloaded_papers.json.
"""
...
def persist_rows(path: Path, rows: list[dict]) -> None:
    tmp = path.with_suffix(".tmp")
    tmp.write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    tmp.replace(path)


def is_completed(row: dict) -> bool:
    pdf_path = row.get("pdf_path", "")
    return bool(row.get("download_ok")) and bool(pdf_path) and Path(pdf_path).exists()
...
def main() -> None:
    ...
    existing_by_pdf_url = load_existing(output_path) if args.resume else {}
    downloaded: list[dict] = []
    reused_count = 0

    for idx, entry in enumerate(entries, 1):
        existing = existing_by_pdf_url.get(entry["pdf_url"])
        ...
        authors: list[str] = existing.get("authors", []) if existing else []
        if not authors:
            try:
        ...
        persist_rows(output_path, downloaded)

    persist_rows(output_path, downloaded)
    print(
        f"Saved {len(downloaded)} records ({reused_count} resumed) -> {output_path}"
    )
